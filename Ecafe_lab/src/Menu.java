...
public class Menu {
	...
	public void addItem() {
		...
		try {
			db_handler.prep_stmt = db_handler.conn.prepareStatement("insert into items (name, category, price) values (?,?,?)");
	...
	public void showMenu() {
		...
		System.out.println("------------------------------------------------------------------");
		try {
			db_handler.prep_stmt = db_handler.conn.prepareStatement("select * from items");
			ResultSet items = db_handler.prep_stmt.executeQuery();
			while(items.next()) {
				System.out.printf("   %-12d%-24s%-22s%-10d",items.getInt("Id"),items.getString("name"),items.getString("category"), items.getInt("price"));
				System.out.println();
			}
			items.close();
			db_handler.prep_stmt.close();
